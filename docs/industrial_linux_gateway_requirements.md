...
> **Estado de plataforma.** Rev. A utilizará **STM32H723VET6** como plataforma principal de procesamiento y **Zephyr RTOS** como base de software embebido. Esta decisión está **ACCEPTED / BASELINE FOR REV A**. La revisión externa del pinout confirmó recursos suficientes para las interfaces previstas, pero la asignación concreta de pines y periféricos sigue **TBD** y deberá congelarse antes del esquemático. La decisión no implica que existan firmware, PCB, prototipo o validación funcional.

---

## 1. Propósito, descripción y alcance

Este documento define el baseline técnico preliminar de un gateway de comunicación industrial destinado a integrar equipos de campo heterogéneos con redes IP, servicios de supervisión, servidores o sistemas SCADA. Conserva los requisitos y decisiones de revisiones anteriores, registra los cambios de arquitectura aceptados y separa explícitamente lo decidido, los objetivos de diseño, lo implementado, lo validado y los elementos todavía TBD.

### 1.1 Problema

Los entornos industriales combinan equipos con RS-485, CAN-FD, señales digitales de 24 V, interfaces Ethernet y dispositivos legacy que no ofrecen integración directa con servicios IP modernos. Las diferencias de interfaz eléctrica, protocolo, referencia de tierra, protección, temporización y mantenibilidad impiden conectar esos equipos directamente a una red superior de forma defendible.

El equipo, la instalación y el flujo que constituirán el caso industrial demostrador concreto todavía son **TBD** (TBD-02/03). No se atribuyen cliente, planta, pérdidas ni métricas sin evidencia.
...
- separar la electrónica expuesta al entorno industrial del dominio de procesamiento embebido.
...
|---|---|
| Modularidad funcional | Mantener límites eléctricos y lógicos documentados entre procesamiento, alimentación, red e interfaces de campo. |
...
| Escalabilidad | Reservar capacidades y expansión externa sin convertirlas en requisitos de Rev. A. |
| Separación de dominios | Mantener el procesamiento embebido separado de transceptores y front-ends expuestos al campo. |
...
- integración del STM32H723VET6 como plataforma principal y Zephyr RTOS como base de software;
- congelación de la asignación de periféricos y pines a partir de la suficiencia ya verificada;
- diseño de la PCB propia y su integración mecánica;
- entrada industrial nominal de 24 VDC, regulación, filtrado y protección;
- una interfaz Ethernet como uplink principal y evaluación de una segunda interfaz como objetivo TBD;
...
- interfaz de servicio desde PC; medio físico y cantidad de interfaces TBD;
- consola o mecanismo de mantenimiento, indicadores de estado y estrategia de watchdog; implementación TBD;
- integración Zephyr, drivers, adquisición y servicios embebidos básicos;
...
- procesador o MCU auxiliar adicional sin requisito que lo justifique.
...
- procesador auxiliar solo si aparece un requisito específico conforme a §3.3.
...
| ID | Hecho |
|---|---|
| H-01 | El STM32H723VET6 es la plataforma principal de procesamiento seleccionada para Rev. A y Zephyr RTOS es la base de software embebido. |
| H-02 | El proyecto incluye el diseño de una PCB industrial propia con separación funcional entre procesamiento e interfaces expuestas al campo. |
...
| H-06 | La tecnología, capacidad, resistencia y arquitectura del almacenamiento persistente permanecen TBD. |
| H-07 | Rev. A no incorpora un procesador o MCU auxiliar adicional sin un requisito que lo justifique. |
| H-08 | Una revisión externa del pinout confirmó que el STM32H723VET6 dispone de pines suficientes para las interfaces previstas; la asignación concreta de señales y periféricos sigue TBD. La referencia durable de esa revisión debe incorporarse al repositorio (TBD-38). |
...
|---|---|---|---|
| D-01 | Arquitectura Linux con separación entre plataforma de cómputo y carrier industrial. | **Superseded by D-18** | Se conserva como decisión histórica. La separación funcional entre procesamiento e interfaces industriales permanece, pero Linux y el SoM dejaron de ser el baseline vigente. |
...
| D-07 | MYC-YM6231 basado en TI AM6231 / AM62x como plataforma de cómputo de Rev. A. | **Superseded by D-18** | Decisión histórica de Rev. 0.3. Se conserva su razonamiento: SoM con Cortex-A53, Cortex-M4F, PRU, Linux, DDR, eMMC y PMIC integrados. Ya no describe la arquitectura vigente. |
| D-08 | No utilizar un MCU auxiliar adicional en Rev. A. | Accepted; clarified by D-18 | Se refiere a un segundo procesador auxiliar. No contradice que la plataforma principal sea ahora un MCU. Solo se reabrirá si aparece una necesidad que el STM32H723VET6 y circuitos de supervisión más simples no puedan satisfacer. |
...
| D-18 | Rev. A utilizará STM32H723VET6 como plataforma principal de procesamiento y Zephyr RTOS como base de software embebido. | **Accepted / Baseline for Rev. A** | Se verificó externamente suficiencia de pines para las interfaces previstas, sin congelar asignaciones concretas. El dispositivo integra Ethernet MAC, CAN-FD y UART/USART suficientes para el alcance planteado; su capacidad de procesamiento se considera adecuada como decisión de arquitectura, pendiente de validación bajo carga. El encapsulado LQFP100 favorece el diseño y fabricación de una PCB propia. Para el alcance actual no se requiere una plataforma Linux completa y una arquitectura embebida dedicada se alinea mejor con las funciones previstas. Coste, consumo, tamaño final, robustez y rendimiento medido no se consideran demostrados. |
...
| C-09 | Supervisor/watchdog externo | Requiere matriz fallo-detector-acción y justificar independencia respecto al MCU principal. |
| C-10 | Segunda interfaz Ethernet | La necesidad, velocidad, arquitectura física y consumo de recursos permanecen TBD. |
| C-11 | Almacenamiento persistente | Tecnología, interfaz, capacidad, resistencia, integridad y retención permanecen TBD. |
...
| S-03 | Zephyr RTOS proporcionará o permitirá integrar los drivers y servicios necesarios para Rev. A. | Verificar versión, soporte de placa/SoC, drivers, subsistemas y restricciones antes del firmware mínimo. |
| S-04 | La suficiencia global de pines del STM32H723VET6 está validada externamente. | Incorporar la referencia durable de la revisión (TBD-38) y congelar la asignación concreta de periféricos y pines antes del esquemático; no inferir señales individuales. |
...
### 3.2 Arquitectura de procesamiento y software embebido

```mermaid
flowchart TB
    FIELD["Periféricos e interfaces de campo<br/>Asignación concreta TBD"]
    MCU["STM32H723VET6<br/>Plataforma principal decidida"]
    ZEPHYR["Zephyr RTOS<br/>Base de software decidida<br/>Firmware not implemented / not validated"]
    SERVICES["Adquisición / validación / normalización<br/>Networking / logging / persistencia<br/>Diagnóstico / configuración / actualización"]
...
|---|---|---|
| Inicialización de Zephyr RTOS y servicios embebidos | Must | Versión, configuración, arranque y criterios TBD; no implementado. |
| Drivers de interfaces habilitadas | Must | Deben validarse contra la asignación de pines y soporte de Zephyr. |
| Adquisición, validación y normalización de datos | Must | Variables, tasas, latencia y formato TBD. |
| Logging, almacenamiento local y cola persistente | Must | Tecnología, capacidad, retención, integridad y resistencia TBD. |
...
| CAN-FD mediante drivers y API soportados | Should; Must si se activa CAN-FD | Driver, bitrate y protocolo de capa superior TBD. |
| MQTT | Should | Broker, QoS, esquema y credenciales TBD. |
| Mantenimiento desde PC | Should | Medio, protocolo, permisos y seguridad TBD. |
...
La lista asigna responsabilidades funcionales a la plataforma STM32H723VET6 + Zephyr. No afirma que el firmware exista ni que todas las funciones `Should` o `Could` se implementarán en Rev. A. MQTT, Modbus TCP, REST y una interfaz web permanecen opciones condicionadas al caso de uso; no son protocolos definitivos por la selección de Zephyr.

### 3.3 Política sobre procesador auxiliar

El STM32H723VET6 es el procesador principal de Rev. A. Rev. A no incorporará otro MCU o procesador auxiliar salvo que aparezca evidencia de una necesidad que no pueda satisfacerse razonablemente con el MCU principal y circuitos de supervisión más simples:

- supervisión independiente del dominio de alimentación del MCU principal;
- power-cycle autónomo del MCU principal;
- requisito de seguridad funcional;
- aislamiento físico de funciones críticas;
- periféricos necesarios no disponibles en la plataforma principal;
- supervisión que deba continuar con el MCU principal apagado.
...
|---:|---|---|---|---|---|
| 1 | Processing platform | Ejecutar firmware Zephyr, adquisición, procesamiento, comunicaciones y diagnóstico. | STM32H723VET6 + Zephyr decided; not implemented / not validated | Asignación concreta, soporte de drivers y presupuesto de recursos | Pin mapping, configuración de Zephyr, memoria y estrategia térmica TBD |
...
| 13 | Supervisor/watchdog strategy | Detectar fallos y ejecutar recuperación con autoridad conocida. | Required strategy; implementación TBD | Matriz fallo-detector-acción | Watchdog del MCU o supervisor externo TBD |
...
| 17 | Persistent storage | Alojar firmware cuando corresponda, configuración, logs y datos pendientes. | Required function; tecnología TBD | Volumen y carga de escritura | Tecnología, interfaz, capacidad, resistencia, organización e integridad TBD |
...
| Dirección | Flujo |
|---|---|
| Adquisición | Field device → physical interface → protection/transceiver/input stage → MCU peripheral → Zephyr driver → validation/protocol handling → normalization → local processing → network service → SCADA/server. Logging o persistencia se aplican en los puntos definidos por la política TBD; no todo paquete debe almacenarse antes de transmitirse. |
| Comando | SCADA/server → Ethernet → Zephyr network service → authorization/validation TBD → protocol handler → MCU peripheral → protected field interface → device |
...
| Firmware and system logs | Must | Persistencia, rotación y retención TBD. |
...
| Controlled remote/local service | Should | Medio, política de acceso y configuración segura TBD. |
...
| UC-04 — Diagnóstico de campo | Distinguir fallos de alimentación, plataforma de procesamiento, electrónica de interfaces, campo y red; consultar versiones/configuración, ejecutar recuperación autorizada y registrar resultado. |
...
| F-08 | El producto deberá proporcionar diagnóstico de alimentación, comunicaciones de campo, plataforma/firmware y enlace superior hasta el nivel soportado. | Variables, acceso y granularidad: TBD | Must | UC-01, UC-04 |
...
| F-16 | El producto no incorporará un procesador o MCU auxiliar adicional en Rev. A sin requisito que lo justifique. | STM32H723VET6 es el MCU principal; supervisión adicional mediante circuitos justificables TBD | Won't | D-08, D-18 |
...
| NF-06 | El producto deberá permitir localizar fallos entre alimentación, plataforma/firmware, electrónica de interfaces, campo y red superior cuando la arquitectura lo permita. | Cobertura diagnóstica: TBD | Should | UC-04 |
...
| NF-08 | La PCB deberá diseñarse para fabricación y prueba mediante procesos documentados acordes con los recursos del proyecto. | Reglas DFM/DFT y cobertura: TBD | Must | UC-06 |
...
Las tablas de requisitos son la referencia normativa. El baseline incluye Ethernet principal; dos RS-485, un CAN-FD, cuatro entradas digitales y dos salidas son objetivos preliminares que orientan la arquitectura, pero no sustituyen el cierre de interfaces, cargas, aislamiento y validación. La segunda interfaz Ethernet y las interfaces analógicas permanecen `TBD` o `Could / Future Expansion`. Evaluar aislamiento o recuperación no impone un componente concreto, un procesador auxiliar ni un supervisor independiente.
...
| SR-PWR-06 | El sistema deberá generar los rails requeridos por el STM32H723VET6 y los circuitos de interfaz. | Tensiones, tolerancias, secuencia, rails y cargas: TBD | Must | D-18, arquitectura | A, T |
| SR-PWR-07 | El sistema deberá llevar MCU, firmware e interfaces a estados definidos durante brownout, pérdida y retorno de alimentación. | Umbrales, tiempos y estados: TBD | Must | UC-03, NF-03 | A, T, D |
...
| SR-PWR-10 | La necesidad de hold-up o apagado controlado deberá resolverse contra el riesgo de corrupción del almacenamiento persistente o de los datos. | Tiempo y estrategia: TBD | Should | UC-03, NF-02 | A, T |
...
| SR-COM-08 | La necesidad de una segunda interfaz Ethernet independiente deberá resolverse antes de congelar la arquitectura de red. | Caso de uso, velocidad, recursos y hardware TBD | Should si se justifica | Reopened / TBD | D-11, C-10 | I, A, T |
| SR-COM-09 | La arquitectura deberá proporcionar un medio de servicio desde PC y evaluar USB cuando sea adecuado. | Medio, cantidad, roles, alimentación y conectores TBD | Should | Target | D-17 | I, A, T |
| SR-COM-10 | La arquitectura deberá contemplar una interfaz de servicio para configuración, logs de arranque, recuperación y debug. | USB, UART, Ethernet u otro medio; nivel, conector, acceso y protección TBD | Should | Target | D-17, UC-04 | I, T, D |
...
### 7.4 Plataforma y software embebido — SR-LNX

El prefijo `SR-LNX` se conserva por trazabilidad histórica. Desde Rev. 0.4 no implica Linux. Los IDs se reformulan o retiran explícitamente, sin reutilizarlos silenciosamente.

| ID | Requisito | Especificación/valor | MoSCoW | Origen | Verificación |
|---|---|---|---|---|---|
| SR-LNX-01 | El STM32H723VET6 deberá contar con soporte Zephyr adecuado para las funciones del ciclo de Rev. A. | Versión Zephyr, board support, drivers y periodo de mantenimiento: TBD | Must | D-18, UC-01 | I, D |
...
| SR-LNX-03 | El sistema deberá proporcionar almacenamiento persistente suficiente para firmware cuando corresponda, registros, configuración y datos temporales. | Tecnología, interfaz, capacidad, resistencia e integridad: TBD | Must | H-06, UC-02 | I, A, T |
...
| SR-LNX-05 | El MCU y su firmware deberán autoarrancar después de restaurar alimentación válida. | Tiempo hasta servicio: TBD | Must | UC-03, D-18 | I, T, D |
| SR-LNX-06 | Zephyr deberá iniciar automáticamente las funciones del gateway y permitir supervisar su estado. | Dependencias, secuencia y recuperación: TBD | Must | UC-03, D-18 | I, T, D |
...
| SR-LNX-08 | El STM32H723VET6 deberá asignar recursos suficientes para Ethernet, buses de campo, E/S, desarrollo y mantenimiento del baseline. | Suficiencia de pines validada externamente; asignación, multiplexación, DMA, interrupciones y concurrencia TBD | Must | D-12, D-13, D-14, D-18, H-08 | I, A, D |
...
| SR-LNX-10 | Zephyr deberá proporcionar o permitir integrar drivers y servicios para las interfaces efectivamente implementadas. | UART/USART, CAN-FD, Ethernet, USB, GPIO y otros según diseño; disponibilidad y configuración TBD | Must | D-18, UC-01 | I, T, D |
...
| SR-LNX-12 | Requisito histórico: ninguna función Must dependería del M4F o PRU del SoM anterior sin validar su integración. | No aplicable a la arquitectura STM32H723VET6 | Retired | D-09, D-10, D-18 | I |

#### 7.4.1 Clasificación de migración de SR-LNX

| Clase | IDs | Tratamiento Rev. 0.4 |
|---|---|---|
| A — Funcional y necesario | SR-LNX-02, 04, 07, 09, 11 | Se conservan con semántica independiente del sistema operativo. |
| B — Implementación Linux anterior | SR-LNX-01, 03, 05, 06, 08, 10 | Se reformulan para STM32H723VET6, Zephyr y almacenamiento genérico, manteniendo los IDs. |
| C — Obsoleto | SR-LNX-12 | Se conserva como `Retired`; no se reutiliza. |
| D — Requiere cierre adicional | SR-LNX-01 a 11 excepto 12 | Versión de Zephyr, drivers, recursos, almacenamiento, actualización, watchdog y presupuestos siguen TBD o pendientes de validación. |
...
| SR-DIA-05 | El sistema deberá distinguir, donde la arquitectura lo permita, fallos de MCU/firmware, electrónica de interfaces, alimentación, campo y Ethernet. | Cobertura y códigos: TBD | Should | UC-04, NF-06 | A, T, D |
...
| SR-DIA-08 | El sistema deberá conservar logs de arranque y firmware conforme a una política que limite pérdida y desgaste del almacenamiento seleccionado. | Medio, retención, rotación y persistencia: TBD | Must | UC-03, UC-04 | I, T, D |
...
| ID | Requisito | Especificación/valor | MoSCoW | Origen | Verificación |
|---|---|---|---|---|---|
| SR-MEC-01 | El conjunto deberá alojar PCB, conectores, cableado y montaje dentro de envolventes definidas. | Dimensiones y montaje: TBD | Must | H-02 | I, A |
...
| SR-MEC-06 | La integración deberá contemplar disipación del MCU, conversión de potencia y etapas de salida con datos de carga reales. | Modelo térmico y límites: TBD | Must | SR-PWR-09 | A, T |
...
| SR-MNT-04 | La documentación deberá registrar compatibilidad entre revisiones de PCB, firmware, configuración y subconjuntos reemplazables definidos. | Matriz de compatibilidad: TBD | Should | D-18, UC-05 | I, D |
...
| ID | Requisito | Especificación/valor | MoSCoW | Origen | Verificación |
|---|---|---|---|---|---|
| SR-TST-01 | La PCB deberá proporcionar acceso de prueba a rails y señales críticas. | Lista, geometría y carga admisible: TBD | Must | UC-06, DFT | I, T |
| SR-TST-02 | Deberá ser posible verificar por separado alimentación, interfaces de campo, MCU/firmware y Ethernet. | Estímulos y criterios: TBD | Must | UC-06 | I, T, D |
...
| SR-TST-05 | La PCB deberá incluir DFT suficiente para detectar fallos de ensamblaje relevantes con recursos disponibles. | Cobertura y recursos: TBD | Must | NF-08 | I, A, T |
...
|---|---|---|---|
| D-18 | Baseline STM32H723VET6 + Zephyr RTOS | F-01 a F-09, F-16 | SR-PWR-06, SR-COM-06, SR-LNX-01 a SR-LNX-11 |
| D-08 | Sin procesador auxiliar en el baseline | F-16 | SR-LNX-09, SR-DIA-03, SR-SAF-01/06 |
| D-11 a D-14 | Interfaces objetivo de Rev. A | F-02, F-10 | SR-COM-01 a SR-COM-10, SR-IO-01 a SR-IO-05 |
| UC-01 | Integrar equipo industrial con red IP | F-01 a F-04, F-08, NF-01 | SR-COM-01 a SR-COM-06, SR-LNX-01/02/08/10/11, SR-DIA-01/06 |
| UC-02 | Conservar y reenviar datos | F-05, F-06, NF-02 | SR-COM-07, SR-LNX-03/04, SR-DIA-02/08 |
| UC-03 | Recuperarse de fallos de alimentación | F-07, NF-03, NF-04 | SR-PWR-01 a SR-PWR-10, SR-LNX-05/06/09, SR-DIA-03, SR-SAF-01/06, SR-PER-03 |
...
Las decisiones D-07, D-09 y D-10 se conservan como historia de la arquitectura MYC-YM6231/Linux y fueron reemplazadas por D-18. La trazabilidad detallada requisito-prueba se desarrollará cuando los TBD relevantes tengan criterios de aceptación.

### 8.1 Trazabilidad NABC: necesidad, arquitectura, beneficio y estado

| Need respaldado | Requisitos | Bloque de arquitectura | Beneficio esperado | Estado de evidencia |
|---|---|---|---|---|
| Integrar RS-485, CAN-FD y señales digitales con una red IP. | F-01 a F-04; SR-COM-01 a SR-COM-06; SR-IO-01 a SR-IO-05 | Field devices → interfaces protegidas → STM32H723VET6/Zephyr → network | Integración de interfaces heterogéneas y adaptación hacia IP. | Decidido como alcance; no implementado; no validado. |
| Conservar información durante interrupciones de conectividad. | F-05 a F-07; SR-COM-07; SR-LNX-03/04 | Procesamiento → almacenamiento persistente TBD → red | Persistencia y reenvío controlado cuando corresponda. | Objetivo de diseño; medio y política TBD; no validado. |
| Detectar y atender fallos del gateway. | F-08/F-09; SR-DIA-01 a SR-DIA-09; SR-MNT-01 a SR-MNT-06 | Diagnóstico/configuración/mantenimiento asociados a la plataforma principal | Mejor capacidad esperada de diagnóstico y mantenimiento. | Objetivo de diseño; no implementado; no validado. |
| Operar con interfaces de campo en un entorno industrial por definir. | NF-09/NF-10; SR-SAF-02 a SR-SAF-04/07; SR-ENV-01 a SR-ENV-06 | Protección/acondicionamiento y aislamiento donde se justifique | Electrónica de campo diseñada alrededor de los riesgos reales del caso de uso. | Requisitos preliminares; niveles y aislamiento TBD. |
| Preparar una Rev. A verificable y fabricable. | NF-08; SR-MEC-03; SR-TST-01 a SR-TST-05 | PCB propia y puntos/medios de prueba por definir | Testabilidad y fabricación consideradas desde requisitos. | Objetivo de diseño; no existe PCB fabricada. |
...
| Integración STM32H723VET6 + Zephyr | Congelar asignación de periféricos/pines y verificar soporte, drivers, concurrencia, arranque y recursos. | Matriz de periféricos/pines, compilaciones reproducibles y pruebas de firmware. |
...
| Prueba de PCB Rev. A | Verificar rails, protecciones, interfaces, I/O, diagnóstico y DFT. | Procedimientos y registros por revisión de hardware. |
| Integración hardware-firmware | Verificar arranque, periféricos, control, diagnóstico y recuperación. | Pruebas reproducibles con versiones identificadas. |
...
|---|---|---|---|---|
| R-01 | La asignación concreta de periféricos, DMA, interrupciones o drivers Zephyr presenta conflictos de concurrencia. | Reducción de alcance o iteración del diseño. | Congelar y revisar el mapa de periféricos/pines antes del esquemático; la suficiencia total de pines ya fue validada externamente. | Abierto |
...
| R-04 | La pérdida de energía corrompe el almacenamiento persistente o los datos pendientes. | Pérdida de servicio o información. | Definir medio, organización de datos, escrituras, brownout, hold-up y power cycling. | Abierto |
...
| R-07 | La arquitectura anterior dependía de M4F/PRU sin validación de integración. | Fallos de integración y cronograma en el baseline anterior. | Riesgo histórico eliminado del baseline por D-18; conservar la evidencia de la decisión. | Cerrado por D-18 |
...
| R-12 | Restricciones mecánicas o térmicas aparecen tarde. | PCB o envolvente incompatibles. | Obtener modelos, montaje, conectores y datos térmicos antes del diseño físico. | Abierto |
| R-13 | Los recursos de memoria, almacenamiento, alimentación o térmica no cubren la carga real. | Revisión del alcance o del diseño de Rev. A. | Elaborar presupuestos y medir una carga representativa antes de congelar producción. | Abierto |
...
Los parámetros sin evidencia se concentran en §12. Aunque la plataforma de Rev. A está seleccionada, no hay asignación concreta de periféricos/pines congelada, equipo demostrador final, esquemático, PCB, firmware validado ni ensayos del gateway completo.
...
| Memoria DDR externa | No requerida por el baseline actual; cualquier necesidad futura exige control de cambios. |
...
## 12. Lista consolidada de TBD y cierres históricos

| ID | TBD por cerrar | Impacto principal | Evidencia necesaria |
|---|---|---|---|
| TBD-01 | Versión de Zephyr, soporte de placa, toolchain, drivers, uso de memoria, alimentación y comportamiento térmico. | Integración de plataforma | Documentación oficial, compilación y pruebas de plataforma. |
...
| TBD-07 | Brownout, hold-up, apagado y recuperación. | Integridad de datos | Capacidades del MCU, arquitectura de alimentación y análisis de almacenamiento. |
...
| TBD-12 | Tiempo de arranque y recuperación. | Disponibilidad | Caso de uso y medición del firmware. |
...
| TBD-17 | I/O digital: niveles, umbrales, cargas, diagnóstico y estados seguros. | Electrónica de campo | Casos de uso y cargas reales. |
...
| TBD-21 | Envolvente, montaje, dimensiones, conectores y grado IP. | Diseño mecánico | Lugar de instalación y selección de componentes. |
...
| TBD-34 | Uso de M4F/PRU de la arquitectura MYC-YM6231 anterior. | Historia de arquitectura | Cerrado/no aplicable al baseline actual por D-18. |
...
| TBD-37 | Presupuesto térmico de MCU, conversión de potencia, interfaces y salidas. | Fiabilidad del diseño | Potencia medida, modelos y prueba térmica. |
| TBD-38 | Referencia durable de la revisión externa que validó la suficiencia global de pines. | Trazabilidad de D-18, H-08 y S-04 | Autor o entidad, fecha, alcance y artefacto revisado; sin inventar asignaciones individuales. |
...
- ¿Existe una necesidad verificable para un segundo Ethernet o debe quedar fuera de Rev. A? (D-11, TBD-27).
- ¿Qué medio de almacenamiento persistente y qué política de retención se usarán? (TBD-07/10/11).
- ¿Qué versión y configuración de Zephyr soportarán las funciones del baseline? (TBD-01).

---

## 14. Próximos pasos de Rev. A
...
| 9 | Desarrollar firmware mínimo en Zephyr. | Bring-up y funciones mínimas reproducibles. |
| 10 | Diseñar PCB Rev. A. | Layout revisado contra requisitos eléctricos, EMC y DFT. |
| 11 | Fabricar Rev. A. | Unidades identificadas para evaluación. |
| 12 | Probar el sistema. | Matriz requisito-evidencia y resultados del demostrador. |
